...
async function loadAdminInfo() {
  try {
    ...
    if (result.status === "success") {
      ...
      document.getElementById("admin-avatar").textContent = admin.full_name
        .charAt(0)
        .toUpperCase();
    }
  } catch (error) {
...
function renderCalendar() {
  ...
  for (let i = 1; i <= daysInMonth; i++) {
    const dayDiv = document.createElement("div");
    dayDiv.className =
      "calendar-day text-center p-2 rounded-lg cursor-pointer hover:bg-blue-100 transition text-gray-900 font-medium";
...
async function loadAppointmentsForDate(dateStr) {
  try {
    const response = await fetch(
      `../../api/admin/appointment_details.php?date=${dateStr}`,
      {
        credentials: "include",
      },
    );
...
document.addEventListener("visibilitychange", async () => {
  if (!document.hidden) {
    ...
    const response = await fetch("../../api/auth/session_info.php", {
      credentials: "include",
    });
