...
function updateStats(stats) {
  ...
  document.getElementById("stat-total-revenue").textContent =
    `$${stats.total_revenue.toLocaleString(undefined, { minimumFractionDigits: 0 })}`;
  document.getElementById("stat-today-tickets").textContent =
    stats.today_tickets.toLocaleString();
  document.getElementById("stat-avg-cost").textContent =
    `$${stats.avg_cost.toLocaleString(undefined, { minimumFractionDigits: 0 })}`;
}
...
function showTicketDetail(ticket) {
  ...
  document.getElementById("modalCost").textContent =
    `$${parseFloat(ticket.cost).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
