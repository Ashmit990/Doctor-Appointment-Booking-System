...
try {
    // Verify appointment
    $stmt = $conn->prepare("SELECT doctor_id, status, feedback FROM appointments WHERE appointment_id = ? AND patient_id = ? FOR UPDATE");
    ...
    if (!empty($apt['feedback'])) {
        throw new Exception('Feedback has already been submitted');
    }

    $doctor_id = $apt['doctor_id'];

    $up = $conn->prepare("UPDATE appointments SET feedback = ? WHERE appointment_id = ?");
    $up->bind_param("si", $feedback, $apt_id);
    $up->execute();
    $up->close();
    ...
    $msg = "Patient {$pat_name} left a feedback review.";
